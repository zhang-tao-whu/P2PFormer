...
class DLASeg(nn.Module):
    def __init__(self, base_name='dla34',
        ...
        cins = [channels[self.first_level]] + channels[self.first_level:self.last_level]
        for i, cin in enumerate(cins):
            self.__setattr__('oproj{}'.format(i),
                             nn.Conv2d(cin, cout, kernel_size=(1, 1), stride=1))
    ...
    def forward(self, x):
        ...
        for i in range(self.last_level - self.first_level):
            y.append(x[i].clone())
            ret.append(x[i].clone())
        self.ida_up(y, 0, len(y))
        ret.insert(0, y[-1])
        for i, feat in enumerate(ret):
            ret[i] = self.__getattr__('oproj{}'.format(i))(feat)
        return ret
